<?php
// api/get_attractions.php
// ══════════════════════════════════════════════════════════
//  Returns all attractions with their image arrays.
//  Supports optional ?city= and ?category= query filters.
//  GET  api/get_attractions.php
//  GET  api/get_attractions.php?city=Bohol
//  GET  api/get_attractions.php?category=Nature
// ══════════════════════════════════════════════════════════
...
// ── IMAGE MAP ─────────────────────────────────────────────
// Maps each attraction id to its image files in the
// attractions/ folder (root-level, outside assets/).
// ─────────────────────────────────────────────────────────
$imageMap = [
...
try {
    ...
    // Attach images, snippet, tripbest, instant to each row
    $attractions = array_map(function ($row) use ($imageMap, $snippetMap, $tripbestMap, $instantMap) {
        ...
        unset($row['image']); // remove single DB image field, replaced by images[]
        return $row;
    }, $rows);

    // Also return unique city list for filter dropdowns
    $cityStmt = $pdo->query('SELECT DISTINCT city FROM attractions ORDER BY city');
